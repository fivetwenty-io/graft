...
| `-v`, `--version` | Yes | Yes | Both print to stdout and exit 0. spruce prints one line, `<argv0> - Version <version>`. graft prints its own line, `graft version <version> (commit: <sha>, built: <timestamp>, go: <toolchain>, os/arch: <goos>/<goarch>)`, and under a `spruce`-named invocation prepends spruce's line byte for byte ahead of it, so a genesis version probe reads the spruce line first. |
