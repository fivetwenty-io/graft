...
| Syntax-forced string quoting | A string that cannot be written as a plain scalar for syntax reasons (a leading `*`, `&`, `%`, an opening bracket, ...) is emitted single-quoted (`'*.uaa.((system_domain))'`); type-lookalike strings (`"1.0"`, `"yes"`, `"null"`) are double-quoted. | Graft classifies each string on marshal (`prefersSingleQuote` in `pkg/graft/yaml.go`): a value goccy's parser rejects outright as a bare scalar, containing no characters that need double-quote escapes, is emitted single-quoted; type-lookalikes still parse as plain scalars (just to a different type) and keep goccy's double quotes, matching spruce. | Matches. Functionally load-bearing for Genesis: its Credhub entombment step regex-replaces `((...))` with `""` in the rendered manifest and re-parses it, which stays valid YAML inside single quotes (`'*.uaa.""'`) but is malformed inside double quotes (`"*.uaa."""`). Pinned by `TestMarshalYAML_SingleQuoteStyle` in `pkg/graft/yaml_spruce_parity_test.go`. |
