...
`VaultConfig.PoolSize` and `AWSConfig.PoolSize` (an addition to the existing `AWSConfig` struct — `Region`, `Profile`, `Role`, `SkipAuth`, `Endpoint`, plus `AccessKeyID`/`SecretAccessKey`/`SessionToken`/`PoolSize`/`MFASerial`/`MFATokenProvider`) set the underlying HTTP transport's `MaxIdleConnsPerHost`/`MaxIdleConns`; non-positive leaves Go's `http.Transport` zero-value default (2 per host) in effect. Retry and caching beyond what the underlying SDKs already do on their own are available by layering `WithBackendRetry("vault", ...)`/`WithBackendCache("vault", ...)` (and the `"awsparam"`/`"awssecret"` equivalents) on top, described in [Custom Backends](../custom-backends.md) — `WithVault`/`WithAWS` deliberately do not duplicate that.

`AWSConfig.MFASerial` (an MFA device ARN/serial) protects `AWSConfig.Role`'s `sts:AssumeRole` call, matching the `Role`-alone case's requirement that `Role` also be set. `AWSConfig.MFATokenProvider` supplies the current code on each call (`aws.NewCredentialsCache` wraps the resulting provider, so it is invoked only when the cached credentials need to refresh, not on every request); leaving it `nil` while `MFASerial` is set falls back to a one-shot code read from the `AWS_MFA_TOKEN` environment variable, or an interactive prompt on stderr when stdin is a terminal, or a clear error naming `AWS_MFA_TOKEN`.

Not carried over from `internal/backends/aws.Target`: `MaxRetries`, `HTTPTimeout`, role-assumption tuning beyond MFA (`AssumeRoleDuration`/`ExternalID`/`SessionName`), `CacheTTL`, `AuditLogging` (the last is available generically via `WithAuditLogger`, described in [Custom Backends](../custom-backends.md), instead). A key/secret ID reaches `Get`/`GetWithTarget` already stripped of its `?stage=...&key=...` query suffix by the operator, so stage/version secret selection is unavailable to a `WithAWS`-registered backend; it always fetches the latest/default version.
