...
Not carried over from `internal/backends/aws.Target`: `MaxRetries`, `HTTPTimeout`, role-assumption tuning beyond MFA (`AssumeRoleDuration`/`ExternalID`/`SessionName`). Audit logging is available generically via `WithAuditLogger`, described in [Custom Backends](../custom-backends.md). A key/secret ID reaches `Get`/`GetWithTarget` already stripped of its `?stage=...&key=...` query suffix by the operator, so stage/version secret selection is unavailable to a `WithAWS`-registered backend; it always fetches the latest/default version.
