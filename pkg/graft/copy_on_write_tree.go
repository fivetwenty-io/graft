...
// Lock acquires a write lock on the tree.
func (cow *COWTree) Lock() {
	cow.mu.Lock()
}

// Unlock releases the write lock on the tree.
func (cow *COWTree) Unlock() {
	cow.mu.Unlock()
}

// RLock acquires a read lock on the tree.
func (cow *COWTree) RLock() {
	cow.mu.RLock()
}

// RUnlock releases the read lock on the tree.
func (cow *COWTree) RUnlock() {
	cow.mu.RUnlock()
}

// Get safely retrieves a value from the tree.
func (cow *COWTree) Get(path string) (interface{}, error) {
	return cow.Find(parsePath(path)...)
}
...
// Set adds or updates a value in the tree using copy-on-write semantics.
func (cow *COWTree) Set(path string, value interface{}) error {
	return cow.SetAtPath(value, parsePath(path)...)
}

// SetAtPath adds or updates a value in the tree using copy-on-write semantics.
