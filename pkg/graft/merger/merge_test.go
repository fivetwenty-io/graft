...
func TestGetArrayModifications(t *testing.T) {
	shouldInsertAt := func(actual interface{}, expIndex ...interface{}) string {
		...
		if !ok {
			return errNotModificationDef
		}
		expIdx, ok := expIndex[0].(int)
		if !ok {
			return "expected index is not int"
		}
		return ShouldEqual(modDef.index, expIdx)
	}
	...
	shouldBeDefault := func(actual interface{}, _ ...interface{}) string {
		...
		if modDef.listOp == listOpMergeDefault {
			return ""
		}
	...
	shouldBeMergeOnKey := func(actual interface{}, _ ...interface{}) string {
		...
		switch modDef.listOp {
		case listOpMergeOnKey:
			return ""
		case listOpMergeDefault, listOpMergeInline, listOpReplace, listOpInsert, listOpDelete:
	...
	shouldBeReplace := func(actual interface{}, _ ...interface{}) string {
		...
		case listOpReplace:
			return ""
		case listOpMergeDefault, listOpMergeOnKey, listOpMergeInline, listOpInsert, listOpDelete:
	...
	Convey("Should recognize string patterns for", t, func() {
		...
		Convey("(( insert ... ))", func() {
			for _, rel := range []string{"before", "after"} {
				...
				for _, key := range []string{"", "foo"} {
					...
					for input, shouldMatch := range map[string]bool{
						fmt.Sprintf(`(( insert %s %s "graft" ))`, rel, key):                true,
						fmt.Sprintf(`(( insert %s %s	 "graft" ))`, rel, key):               true,
						fmt.Sprintf(`(( insert	 %s %s "graft" ))`, rel, key):               true,
						fmt.Sprintf(`(( insert   %s %s   "graft" ))`, rel, key):            true,
						fmt.Sprintf(`((   insert %s %s "graft" ))`, rel, key):              true,
						...
						fmt.Sprintf(`((    insert    %s    %s     "graft"   ))`, rel, key): true,
						fmt.Sprintf(`((   insert   %s %s   "graft"	 ))`, rel, key):         true,
						fmt.Sprintf("(( insert%s%sgraft ))", rel, key):                     false,
						fmt.Sprintf("(( insert%s%s graft ))", rel, key):                    false,
						...
						Convey(fmt.Sprintf("with case %s", input), func() {
							...
							if shouldMatch {
								So(results, ShouldHaveLength, 2)
								So(results[0], shouldBeDefault)
								So(results[1].name, ShouldEqual, "graft")
								testKey := key
								if testKey == "" {
									testKey = "name"
								}
		...
		Convey("(( delete ... ))", func() {
			for _, key := range []string{"", "foo"} {
				...
				for input, shouldMatch := range map[string]bool{
					fmt.Sprintf(`(( delete %s "graft" ))`, key):        true,
					fmt.Sprintf(`(( delete %s	 "graft" ))`, key):       true,
					fmt.Sprintf(`(( delete	 %s "graft" ))`, key):       true,
					fmt.Sprintf(`(( delete   %s   "graft" ))`, key):    true,
					fmt.Sprintf(`((   delete %s "graft" ))`, key):      true,
					...
					fmt.Sprintf(`((    delete %s "graft" ))`, key):     true,
					fmt.Sprintf(`((   delete   %s   "graft"	 ))`, key): true,
					fmt.Sprintf("(( delete%sgraft ))", key):            false,
...
func TestMergeObj(t *testing.T) {
	...
	Convey("Passing a map to m.MergeObj merges as a map", t, func() {
		...
		Convey("returns an error for any (( merge ... )) operators found in non-list context", func() {
			...
			err := m.Error()
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldContainSubstring, "node-path.map: inappropriate use of (( merge )) operator outside of a list (this is graft, after all)")
		})
	})
