...
func TestCOWEvaluator(t *testing.T) {
	Convey("COWEvaluator operations", t, func() {
		...
		Convey("Snapshot creation", func() {
			_ = evaluator.SetValue("original", "snapshot", "value")
			...
			// Modify original
			_ = evaluator.SetValue("modified", "snapshot", "value")
		...
		Convey("Version tracking", func() {
			...
			_ = evaluator.SetValue("test", "version", "key")
...
func TestEnhancedMigrationHelper(t *testing.T) {
	Convey("EnhancedMigrationHelper operations", t, func() {
		...
		Convey("Snapshot management", func() {
			...
			_ = helper.GetCOWEvaluator().SetValue("modified1", "meta", "name")
			snapshot2 := helper.CreateSnapshot()

			_ = helper.GetCOWEvaluator().SetValue("modified2", "meta", "name")
	...
	Convey("COWEvaluator concurrent operations", t, func() {
		...
		Convey("Concurrent snapshots and modifications", func() {
			...
			for i := 0; i < 10; i++ {
				...
				go func(id int) {
					...
					for j := 0; j < 10; j++ {
						...
						// Modify evaluator
						_ = evaluator.SetValue(fmt.Sprintf("worker-%d-%d", id, j), "counters", fmt.Sprintf("key%d", id))
					}
				}(i)
			}
			...
			case <-time.After(10 * time.Second):
				t.Fatal("Test timed out - possible deadlock in concurrent snapshot creation")
			}
		...
		Convey("Concurrent evaluator operations", func() {
			...
			case <-done:
				close(errors)
			case <-time.After(10 * time.Second):
				t.Fatal("Test timed out - possible deadlock in concurrent evaluator operations")
			}
...
func BenchmarkCOWEvaluator(b *testing.B) {
	...
	b.Run("SetValue", func(b *testing.B) {
		b.RunParallel(func(pb *testing.PB) {
			...
			for pb.Next() {
				...
				_ = evaluator.SetValue(fmt.Sprintf("value%d", i), "data", key)
				i++
			}
		})
	})
	...
	b.Run("GetValue", func(b *testing.B) {
		b.RunParallel(func(pb *testing.PB) {
			...
			for pb.Next() {
				...
				_, _ = evaluator.GetValue("data", key)
				i++
			}
		})
	})
	...
	b.Run("SnapshotAndModify", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			snapshot := evaluator.CreateSnapshot()
			_ = snapshot.SetValue(i, "snapshot", "value")
		}
	})
}
