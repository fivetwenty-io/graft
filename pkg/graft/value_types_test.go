...
func TestValue(t *testing.T) {
	t.Run("String value", func(t *testing.T) {
		...
		if str != testHelloStr {
			t.Errorf("expected 'testHelloStr', got '%s'", str)
		}
...
func TestTypedResponse(t *testing.T) {
	t.Run("Create and convert", func(t *testing.T) {
		tr := NewTypedResponse(Replace, testHelloStr)
		...
		if legacy.Value != testHelloStr {
			t.Errorf("expected '%s', got '%v'", testHelloStr, legacy.Value)
		}
		...
		if str2 != testHelloStr {
			t.Errorf("expected '%s', got '%s'", testHelloStr, str2)
		}
	})
}
