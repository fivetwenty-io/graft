	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.pos.Line != 0 && tt.pos.Column != 0 {
				if colonIdx := strings.Index(tt.expected, ":"); colonIdx != -1 && tt.pos.File != "" && tt.pos.File != tt.expected[:colonIdx] {
...
func TestExprError_Constructors(t *testing.T) {
	...
	t.Run("WrapError", func(t *testing.T) {
		...
		if !errors.Is(wrapped.Nested, original) {
