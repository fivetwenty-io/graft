...
const (
	testYAMLKey1Value1 = "key1: value1"
)
...
func (m *TestMockOperator) Dependencies(ev *Evaluator, args []*Expr, locs, auto []*tree.Cursor) []*tree.Cursor {
	return nil
}
...
`
		...
		SkipConvey("Evaluate should fail for non-map document", func() {
			// Document interface doesn't allow creating documents with non-map data
			// directly. This would need to be tested at a lower level where we can
			// create invalid document structures.
		})
	})
}
...
func TestEngineOperatorRegistryMethods(t *testing.T) {
	Convey("Engine Operator Registry Methods", t, func() {
		...
		Convey("RegisterOperator should fail for existing operator", func() {
			...
			_ = engine.RegisterOperator("test_op_duplicate", mockOp)
...
func TestEngineMerging(t *testing.T) {
	Convey("Engine Merging", t, func() {
		...
		Convey("Merge should handle nil context", func() {
			...
			builder := engine.Merge(context.TODO(), doc1)
			So(builder, ShouldNotBeNil)
		})
...
func TestEngineErrorHandling(t *testing.T) {
	Convey("Engine Error Handling", t, func() {
		...
		Convey("should handle invalid operator expressions gracefully", func() {
			...
			// Engine is lenient - unknown operators don't cause crashes
			// The expression may be left unevaluated or treated as a literal string
			So(result, ShouldNotBeNil)
			// If no error, verify engine completed successfully
			if err == nil {
				// Result should contain the original expression or an evaluated form
				val, getErr := result.Get("result")
		...
		Convey("should handle malformed operator expressions gracefully", func() {
			...
			// Engine is lenient - malformed syntax doesn't cause crashes
			So(result, ShouldNotBeNil)
			// If no error, verify engine completed successfully
			if err == nil {
				val, getErr := result.Get("result")
...
func TestCreateEngineFromOptions(t *testing.T) {
	Convey("createEngineFromOptions", t, func() {
		...
		Convey("should allow custom operator to override built-in", func() {
			mockOp := NewTestMockOperator("grab")
			opts := &EngineOptions{
				CustomOperators: map[string]Operator{
					"grab": mockOp, // Custom operators can shadow/override built-ins
				},
			}
			...
			// Verify the custom operator is registered
			op, exists := engine.GetOperator("grab")
			So(exists, ShouldBeTrue)
			So(op, ShouldEqual, mockOp)
		})
	})
